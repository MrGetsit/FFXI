...
function user_setup()		
	...
	send_command('send @all bind %e   sta Spikex /Dia3 <stnpc>')
	...
	send_command('send @all bind ~%3 send Spikex /GainMnd')
	...
	send_command('send @all bind ~%7 send Spikex /Stoneskin')
	send_command('send @all bind ~%8 send Spikex /Refresh3 <stpc>')
	...
	send_command('send @all bind %` send Spikex /Sleep')
	send_command('send @all bind %~` send Spikex /Break')
	--send_command('send @all bind %c  send Spikex /Chainspell')
	...
	if player.sub_job == 'SCH' then
		...
		send_command('send @all bind %x send Spikex /Manifestation')
		send_command('send @all bind !x send Spikex /DarkArts')
...
function init_gear_sets()
    ...
    --- Other Sets ---
    sets.idle = set_combine(sets.defense, { head="Null Masuqe", ammo="Homiliary"})
